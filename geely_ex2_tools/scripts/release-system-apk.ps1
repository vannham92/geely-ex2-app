...
# Detect OS
$isWin = $IsWindows
if ($null -eq $isWin) { $isWin = $env:OS -like "*Windows*" }

# Setup Java on macOS if needed
if (-not $isWin -and -not $env:JAVA_HOME) {
    $jbr = "/Applications/Android Studio.app/Contents/jbr/Contents/Home"
    if (Test-Path $jbr) {
        $env:JAVA_HOME = $jbr
        $env:PATH = "$jbr/bin" + [IO.Path]::PathSeparator + $env:PATH
        Write-Host "==> using bundled JBR: $jbr"
    }
}

# Determine script root and set location
$scriptPath = $MyInvocation.MyCommand.Path
if (-not $scriptPath) { $scriptPath = Get-Location } # Fallback for some shell environments
...
& $gradleCmd :app:assembleSystemRelease --quiet
...
if (-not $sdk) {
    if ($isWin) {
        ...
        $sdk = "$env:HOME/Library/Android/sdk"
    }
}

$apksignerName = if ($isWin) { "apksigner.bat" } else { "apksigner" }
$zipalignName = if ($isWin) { "zipalign.exe" } else { "zipalign" }

$apksigner = Get-ChildItem "$sdk/build-tools" -Recurse -Filter $apksignerName |
    Sort-Object FullName -Descending |
    Select-Object -First 1 -ExpandProperty FullName
$zipalign = Get-ChildItem "$sdk/build-tools" -Recurse -Filter $zipalignName |
    Sort-Object FullName -Descending |
    Select-Object -First 1 -ExpandProperty FullName

if (-not $apksigner) { throw "$apksignerName not found under $sdk/build-tools" }
...
if ($isWin) {
    ...
    & $apksigner verify --print-certs $signedNamed | Select-String "Signer #1 certificate"
}
$ErrorActionPreference = "Stop"
